 ...
 *   `[null, charSetSize]`.
 */
export const createModel = (sampleLen: number, charSetSize: number, lstmLayerSizes: number | number[]): tf.LayersModel => {
...
export const generateText = async (
    model: tf.LayersModel, textData: TextData, sentenceIndices: number[], length: number, temperature: number,
    onTextGenerationChar: (char: string) => Promise<void>): Promise<string> => {
    const sampleLen = model.inputs[0].shape[1] as number
    const charSetSize = model.inputs[0].shape[2] as number
