...
const loadFeatures = async (filePath: string, numWords: number, maxLen: number,
    multihot = false): Promise<tf.Tensor | void> => {
    const buffer = await fetchLocal(filePath)
    if (!buffer) {
        return
    }
 ...
 */
const loadTargets = async (filePath: string): Promise<tf.Tensor2D | void> => {
    const buffer = await fetchLocal(filePath, false)
    if (!buffer) {
        return
    }
...
export const loadMetadataTemplate = async (): Promise<void> => {
    const buffer = await fetchLocal(METADATA_TEMPLATE_URL) as Buffer
